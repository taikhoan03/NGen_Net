...
{
    public class Record_upc_rsd
    {
        public static readonly log4net.ILog logger = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public DataObject.QueryResult Insert(DataObject.Record_UPC_RSD record)
        {
            try
            {
                var new_record = new NexGenFlow.NexGen.doc_upc_rsd(new Session());
        ...
        public DataObject.QueryResult Insert(DataObject.Record_UPC_RSD record, Session session)
        {
            try
            {
                var new_record = new NexGenFlow.NexGen.doc_upc_rsd(session);
                //new_record.image_2 = record.Image_2;
                new_record.upc_code = record.Upc_code;
                new_record.description = record.Description;
                new_record.per_item = record.Per_Item;
        ...
        public DataObject.QueryResult Insert(List<DataObject.Record_UPC_RSD> records,long documentid,long keying_time,string username,string transaction_date, string transaction_time,double total_update)
        {
