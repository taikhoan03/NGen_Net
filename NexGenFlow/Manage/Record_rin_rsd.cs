...
{
    public class Record_rin_rsd
    {
        public static readonly log4net.ILog logger = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public DataObject.QueryResult Insert(DataObject.Record_RIN_RSD record)
        {
            try
            {
                var new_record = new NexGenFlow.NexGen.doc_rin_rsd(new Session());
                new_record.image_2 = record.Image_2;
                new_record.item_number = record.Item_number;
        ...
        public DataObject.QueryResult Insert(DataObject.Record_RIN_RSD record, Session session)
        {
            try
            {
                var new_record = new NexGenFlow.NexGen.doc_rin_rsd(session);
                new_record.image_2 = record.Image_2;
                new_record.item_number = record.Item_number;
                new_record.description = record.Description;
                new_record.per_item = record.Per_Item;
        ...
        public DataObject.QueryResult Insert(List<DataObject.Record_RIN_RSD> records, long documentid, long keying_time, string username, string transaction_date, string transaction_time,double total_update)
        {
            var session = new Session();
            session.BeginTransaction();
            try
            {
                //check is still locked this doc
                ...
                cl_lockeddoc.TopReturnedObjects = 1;
