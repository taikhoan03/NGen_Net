            ...
            if (package != null)
            {
                ...
                if (docs != null)
                {
                    ...
                    try
                    {
                        ...
                        var data_docs_rsd = new List<DataObject.doc_rsd_2>();
                        if(package.doctype==DataObject.EVRData.Transaction_type_RSD)
                        data_docs_rsd=(List<DataObject.doc_rsd_2>)db.ExecuteReader<DataObject.doc_rsd_2>("SELECT * from doc_rsd where " + new_str_condiction, false);



                        var data_doc_upc = new List<DataObject.doc_upc_2>();
                        if (package.doctype == DataObject.EVRData.Transaction_type_UPC)
                            data_doc_upc = (List<DataObject.doc_upc_2>)db.ExecuteReader<DataObject.doc_upc_2>("SELECT * from doc_upc where " + new_str_condiction, false);

                        var data_doc_rin = new List<DataObject.doc_rin_2>();
                        if (package.doctype == DataObject.EVRData.Transaction_type_RIN)
                            data_doc_rin = (List<DataObject.doc_rin_2>)db.ExecuteReader<DataObject.doc_rin_2>("SELECT * from doc_rin where " + new_str_condiction, false);
                        //
                        var data_doc_rin_rsd = new List<DataObject.doc_rin_rsd_2>();
                        if (package.doctype == DataObject.EVRData.Transaction_type_RIN_RSD)
                            data_doc_rin_rsd = (List<DataObject.doc_rin_rsd_2>)db.ExecuteReader<DataObject.doc_rin_rsd_2>("SELECT * from doc_rin_rsd where " + new_str_condiction, false);

                        var data_doc_upc_rsd = new List<DataObject.doc_upc__rsd_2>();
                        if (package.doctype == DataObject.EVRData.Transaction_type_UPC_RSD)
                            data_doc_upc_rsd = (List<DataObject.doc_upc__rsd_2>)db.ExecuteReader<DataObject.doc_upc__rsd_2>("SELECT * from doc_upc_rsd where " + new_str_condiction, false);
                        ...
                        foreach (var doc in docs)
                        {
                            ...
                            else
                            {
                                if (package.doctype == DataObject.EVRData.Transaction_type_RIN)
                                {
                                    var docs_rin_sub = data_doc_rin.Where(p => p.docid == doc.id).OrderBy(p => int.Parse(p.sr_no));
                                ...
                                //
                                else if (package.doctype == DataObject.EVRData.Transaction_type_UPC_RSD)
                                {
                                    var docs_upc_sub = data_doc_upc_rsd.Where(p => p.docid == doc.id).OrderBy(p => int.Parse(p.sr_no));
                                    foreach (var item in docs_upc_sub)
                                    {
                                        //var doc = docs.Where(p => p.id == item.docid).FirstOrDefault();
                                        if (doc != null)
                                        {
                                            ...
                                            //    record.Rejection_reason = discard_doc.remarks;
                                            record.Upc = item.upc_code;
                                            record.Rin_upc_rsd_description = item.description;
                                            record.Item_type = item.item_type;
                                            record.Sr_no = int.Parse(item.sr_no);
                                            record.Total = doc.total;
                                            record.Transaction_date = doc.transaction_date;
                                ...
                                if (package.doctype == DataObject.EVRData.Transaction_type_RIN_RSD)
                                {
                                    var docs_rin_sub = data_doc_rin_rsd.Where(p => p.docid == doc.id).OrderBy(p => int.Parse(p.sr_no));
                                    foreach (var item in docs_rin_sub)
                                    {
                                        //var doc = docs.Where(p => p.id == item.docid).FirstOrDefault();
                                        if (doc != null)
