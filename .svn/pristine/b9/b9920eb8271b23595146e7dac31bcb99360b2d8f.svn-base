...
{
    public class Record_rin
    {
        public static readonly log4net.ILog logger = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public DataObject.QueryResult Insert(DataObject.Record_RIN record)
        {
            try
            {
                var new_record = new NexGenFlow.NexGen.doc_rin(new Session());
        ...
        public DataObject.QueryResult Insert(DataObject.Record_RIN record, Session session)
        {
            try
            {
                var new_record = new NexGenFlow.NexGen.doc_rin(session);
                new_record.image_2 = record.Image_2;
                new_record.item_number = record.Item_number;
                new_record.per_item = record.Per_Item;
                new_record.price = record.Price;
        ...
        public DataObject.QueryResult Insert(List<DataObject.Record_RIN> records, long documentid, long keying_time, string username, string transaction_date, string transaction_time,double total_update)
        {
